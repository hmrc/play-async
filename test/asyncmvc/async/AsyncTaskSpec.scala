...
import asyncmvc.controllers.FakePlayApplication
...
class AsyncTaskSpec extends UnitSpec with FakePlayApplication with ScalaFutures with Eventually {
  ...
  "AsyncTask" should {
    ...
    "return Complete state when the off-line future completes successfully" in new Complete {
      // Note: Future has 1 second delay time and reason for 2 second delay in eventually.
